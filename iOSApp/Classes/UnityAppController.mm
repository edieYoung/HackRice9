...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    _pic = [[PickImageController alloc] init];
...
- (void)startSelfIOSView
{
    ...
    //vc.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"bg.jpg"]];
    vc.view.layer.contents = (id)[UIImage imageNamed:@"bg.jpg"].CGImage;
    vc.view.frame = [UIScreen mainScreen].bounds;
    UITextView *subTitle = [[UITextView  alloc] initWithFrame:CGRectMake(35, 100, 300, 50)];
    subTitle.text = @"Fit Your Place";
    subTitle.allowsEditingTextAttributes = NO;
    subTitle.textColor = [UIColor blackColor];
    subTitle.backgroundColor = [UIColor clearColor];
    [subTitle setFont:[UIFont fontWithName:@"Courier" size:30]];
    UITextView *title = [[UITextView alloc] initWithFrame:CGRectMake(35, 70, 300, 50)];
    title.text = @"ARoom";
    title.textColor = [UIColor blackColor];
    title.allowsEditingTextAttributes = NO;
    title.backgroundColor = [UIColor clearColor];
    [title setFont:[UIFont fontWithName:@"Courier" size:40]];
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(50, 150, 200, 30)];
    btn.backgroundColor = [UIColor blackColor];
    [btn setTitle:@"Choose environment" forState:UIControlStateNormal];
    [btn addTarget:self action:@selector(openGallary) forControlEvents:UIControlEventTouchUpInside];
    UIButton *start = [[UIButton alloc]initWithFrame:CGRectMake(120, 190, 70, 30)];
    start.backgroundColor = [UIColor blackColor];
    [start setTitle:@"Start" forState:UIControlStateNormal];
    [start addTarget:self action:@selector(startUnity:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)openGallary
{
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary])//相册
    {
        ...
        picker_library_.sourceType = sourceType;
        picker_library_.allowsEditing = YES;
        //delegate
        picker_library_.delegate = _pic;
        
        [_pic presentViewController:picker_library_ animated:YES completion:^{
        }];
        _pic.view.backgroundColor = [UIColor clearColor];
        [_window addSubview:_pic.view];
        
        NSLog(@"picking image");
        
    }else{
        NSLog(@"error with library");
    }
}
